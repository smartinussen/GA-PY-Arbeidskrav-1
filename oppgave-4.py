...
def count_pr_category(datalist: list[Case]) -> dict[str, int]:
    cat_count = {}
    for row in datalist:
        if not row['category'] in cat_count:
            ...
            for row_num, row in enumerate(reader, start=2):
                ...
                for column, value in row.items():
                    if value is None or str(value).strip() == "":
                        print(f"Missing value in row {row_num} - column: '{column}'. Row skipped")
                        row_is_valid = False
                        break
                if not row_is_valid:
                    continue
                try:
                    ...
                    row['id'] = id_no
                    if id_no < 1:
                        print(f"Invalid value in row {row_num} - 'id' = {row['id']} - Must be 1 or higher. Row skipped")
                        continue
                except ValueError:
                    print(f"Invalid value in row {row_num} - 'id' = {row['id']} - Must be a positive integer. Row skipped")
                    continue
                try:
                    mins = int(row['minutes'])
                    row['minutes'] = mins
                    if mins < 0:
                        print(f"Invalid value in row {row_num} - 'minutes' = {row['minutes']} - Must be 0 or more. Row skipped")
                        continue
                except ValueError:
                    print(f"Invalid value in row {row_num} - 'minutes' = {row['minutes']} - Must be a positive integer. Row skipped")
                    continue
                if not row['is_resolved'] in ("yes", "no"):
                    print(f"Invalid value in row {row_num} - 'is_resolved' = {row['is_resolved']} - Must be 'yes' or 'no'. Row skipped")
                    continue
...
def write_report(validated_cases: list[Case] | None) -> None:
    ...
    with open("support-rapport.txt", "w", encoding="utf-8") as report:
        ...
        report.write(f"Total tid brukt: {hours_spent} timer, og {minutes_spent} minutter\n")
        report.write(f"Gj.snitt tidsbruk pr henvendelse: {avg_mins_pr_call:.1f} minutter\n")
        ...
        report.write(f"Det er totalt {unresolved_calls} uløste- og {resolved_calls} løste saker\n")
        ...
        sorted_unresolved = get_call_status(sort_case_by_duration(validated_cases), "no")
        for row in sorted_unresolved:
            report.write(f"Id {row['id']} er uløst med {row['minutes']} minutter brukt.\n")
